---
title: "PQAC Parallel Proving --- Pseudo-algorithm"
author: "PQAC-EUDIW"
date: "2026-06-06"
geometry: margin=1in
fontsize: 11pt
...
| $\{0,1\}^{*}$ | bit string of any length |
| $x \mathbin{\Vert} y$ | concatenation |
| $x \oplus y$ | bitwise XOR ($x, y$ same length) |
| $B[i{:}j]$ | bytes $i$ (incl.) $\dots$ $j$ (excl.) of byte string $B$ |
| $\mathrm{int}(B)$ | big-endian integer value of byte string $B$ |
...
$$
\begin{aligned}
\mathrm{Proof} \;=\;& \{0,1\}^{*} && \text{a zero-knowledge proof} \\[6pt]
X_S \;=\;& (PK_{II}:\{0,1\}^{*},\; m:\mathbb{F},\; k_v:\{0,1\}^{256},\; R_S:\{0,1\}^{256}) && \text{STARK instance (public)} \\
W_S \;=\;& (sign:\{0,1\}^{*},\; e_1:\{0,1\}^{256},\; k_p:\{0,1\}^{256}, && \text{STARK witness (private)} \\
...
W_L \;=\;& (r_2:\{0,1\}^{256},\; s_2:\{0,1\}^{256},\; MSO:\{0,1\}^{*}, && \text{Ligero witness (private)} \\
        ...
        & \phantom{(}\, a:\{0,1\}^{128},\; b:\{0,1\}^{128}) \\[6pt]
Pres \;=\;& (X_S,\; X_L,\; \pi_S:\mathrm{Proof},\; \pi_L:\mathrm{Proof}) && \text{presentation to the verifier}
\end{aligned}
$$
...
## 5. Proof systems (each bundles its relation with its prover/verifier)

Each prover is a **non-interactive zero-knowledge argument of knowledge (NIZKAoK)**,
given here as one self-contained `module`. A module contains:

- $R(x, w) \to \{0,1\}$ --- the **relation predicate**: returns $1$ iff the private
  witness $w$ is valid for the public instance $x$ (clause labels $(S{*})$/$(L{*})$
  tag the conjuncts);
- $\mathrm{Prove}(x, w) \to \pi$ and $\mathrm{Verify}(x, \pi) \to b$ --- the **abstract** prover/verifier
  (bodies are the underlying argument system: Winterfell STARK / Frigo--Shelat
  Ligero), pinned to $R$ by a `require` precondition and an `ensure` postcondition.

$R$ is never executed during the protocol; it is the specification $\mathrm{Prove}$/$\mathrm{Verify}$
are built against. The $\mathrm{Prove}$/$\mathrm{Verify}$ contract is, for both modules:
completeness ($R(x,w)=1 \Rightarrow \mathrm{Verify}(x, \mathrm{Prove}(x,w))=1$), knowledge soundness
($\mathrm{Verify}(x,\pi)=1 \Rightarrow$ the prover knew $w'$ with $R(x,w')=1$), and zero-knowledge
($\pi$ reveals nothing about $w$ beyond $\exists w.\, R(x,w)=1$).
