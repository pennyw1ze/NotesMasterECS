...
We require the issuer to adopt a slightly modified version of dilithium NIST standardized post quantum signature scheme, and to sligthly leverage the credentials structure in order 
...
we perform a full ananlysis of the actual state of the art on device bounded anonymous 
...
\begin{definition}~\cite{Hoffstein2008Cryptography}
...
\begin{definition}~\cite{Huang2025GroupTheory}
...
\begin{definition}~\cite{Menezes1996Handbook}
...
signature schemes. In particular, the actual most efficient and widely adopted
signature scheme is ECDSA~\cite{AlZubaidie2019Efficient}, which stands for Elliptic Curve Digital Signature 
Algorithm. 
...
credential's issuers adopt this type of signature scheme right now.

The problem with ECDSA is that it is not post-quantum secure, meaning that 
it can be broken by a quantum computer.

In order for our thesis to be postquantum secure, we need to adopt a 
...
quantum computer using Shor's algorithm~\cite{Shor1994Algorithms}.
Since Shor's algorithm has been discovered in 1994, many post-quantum
...
\begin{tabular}{|l|l|p{4.5cm}|p{6cm}|}
\hline
\textbf{Scheme} & \textbf{Standard} & \textbf{Underlying Problem} & \textbf{Brief Explanation} \\ \hline
\textbf{ML-DSA} (Dilithium) & FIPS 204 & Module Learning with Errors (MLWE) \& Module Short Integer Solution (MSIS) & Based on the ``Fiat-Shamir with Aborts'' framework. It creates a signature by generating a commitment and response, applying rejection sampling to ensure the output does not leak information about the private key. \\ \hline
\textbf{SLH-DSA} (SPHINCS+) & FIPS 205 & Collision and pre-image resistance of cryptographic hash functions & A stateless hash-based signature scheme. It uses a hyper-tree of Merkle trees to authenticate underlying one-time (WOTS+) and few-time (FORS) signature key pairs. \\ \hline
\textbf{FN-DSA} (FALCON) & Draft FIPS 206 & NTRU Lattice Problem & Uses a hash-and-sign framework with fast-Fourier sampling over NTRU lattices. It produces highly compact signatures, though the signing process requires complex floating-point arithmetic. \\ \hline
\end{tabular}
\end{table}
...
\section{Reproducibility: build and run commands}
\label{results:repro}
...
\section{Unmodified building blocks}
...
\section{Choosing the cross-system commitment: SHA-256 vs Poseidon}
\label{results:hash}
...
\subsection{How bad is SHA-256 in STARK? A proof-size sensitivity study}
