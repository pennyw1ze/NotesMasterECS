...
\begin{itemize}
  ...
  \item \textbf{Pre-image resistance}: Given a hash value, it should be computationally i
  nfeasible to find the original input;
...
there are 2 foundamental problems in lattice-based cryptography~\cite{Hoffstein2008Cryptography}:
...
\end{itemize}
Zero-knowledge proofs are a widley adopted tool in the actual cryptography landscape,
...
In the table~\ref{tab:zk_systems_pq_status}, we summarize the main zero-kwowledge proof systems that are used
in the actual cryptography landscape, with their underlying mathematical problem and need of trusted setup,
enphasizing their post-quantum security.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{|l|l|L|L|l|}
    \hline
    \textbf{Framework} & \textbf{Authors \& Release} & \textbf{Underlying Problem} & \textbf{Trusted Setup} & \textbf{PQ Security} \\ \hline
    ...
    \textbf{Plonk}~\cite{Gabizon2019Plonk}     & Gabizon et al. (2019)    & Polynomial Commitment Schemes (e.g., KZG Pairings) \& Grand Product Arguments & Required (Universal \& Updatable) & Vulnerable \\ \hline
    \textbf{RISC Zero}~\cite{RiscZero2026}   & RISC Zero Team (2022)    & RISC-V Instruction Set Architecture Emulation \& STARK Protocol (DEEP-ALI) & None (Transparent) & Secure \\ \hline
...
Ligoer is based on the following advanced cryptographic thechniques:
\begin{itemize}
