...
\subsection{The Concept of Level of Assurance (LoA) High}
\label{loa}
...
2015/1502.
\subsubsection{Definition of LoA High}
...
\subsubsection{LoA High in the EUDI Wallet Ecosystem}
...
of using Ligero as main actor together with zk-dilithium library and MAC code witness passing techinque.
% List of attempts:
...
Still keeping in mind our design choiche to be compliant with the actual standards, we focused on the possibility to implement
...
run on android where extremly inefficient and not suitable for our purposes. We had to find another approach to solve the problem.

\paragraph{zkDilithium library}
This is where we had a foundamental intuition, which is actually one of the main contributions of this thesis to the current
state of the art. 
\begin{claim}
Post-quantum security against a passive quantum adversary does not require the device binding signature algorithm to be post-quantum secure.
\end{claim}
...
\end{itemize}
Given this optimizations, the results that the authors report in their paper are the provided in the table~\ref{tab:runtimes_memory} below.
...
common commitment scheme.Immeditaly, we notice that this commitment should have 2 properties:
\begin{itemize}
...
of the sha256 preimage prover in Ligero, and, as Frigo and Shelat shows in thier paper~\cite{cryptoeprint:2024/2010}, the sha256 preimage
...
\end{tabularx}
\end{table}
The problem there became the following: how to implement sha256 inside the STARK prover system ? The answare was not trivial at all.
...
sha256 of the credential $x$, where $a$ and $b$ are obteined as $(a,b) = k$ with respect to the key $k = k_p \oplus k_v$. 
...
% ═══════════════════════════════════════════════════════════════════════════
\subsubsection{Assumed Components}
...
% ═══════════════════════════════════════════════════════════════════════════
\subsubsection{Algorithm 1 --- Orchestrator}
...
% ═══════════════════════════════════════════════════════════════════════════
\subsubsection{Algorithm 2 --- Verifier}
\label{sec:verifier}
% ═══════════════════════════════════════════════════════════════════════════
...
\label{analysis}
In this seciton we are going to perform analysis with respect to the security of our solution, we are going to analyze
...
\subsubsection{MAC witness sharing}
...
secure against semihonest adversaries, as shown before.
Why our solution is not actual secure against malicious adversaries?
...
he will be able to pass his credentials across different deiveces and therefore he could allow other
...
In conclusion, the solution proposed in this thesis is secure against semihonest quantum adversarial prover,
but is a perfect bridge between a completly quantum-agnostic implementation of the wallet and a future
...
Post-quantum sound solutions are likely to be out soon, since the enitre online comunity, due to incurrent quantum
threat, is moving towards post-quantum secure softwares. In particular, Google declared that the next version
...
parallel. The goal is to give the reader a faithful picture of \emph{why} each
design decision described in \cref{implementation} was taken, backed by numbers
on real hardware.
...
(\texttt{age\_over\_18}), and the prover key share $k_p$ is fixed for
reproducibility. Reported security levels are the conjectured ones: $115$~bit for
the zkDilithium STARK, ${\approx}96$~bit for the standalone SHA-256 STARK, and
the Longfellow default for the Ligero proofs.

\subsection{Unmodified building blocks}
\label{results:baselines}
\Cref{tab:res-baselines} reports the two systems we build upon, without any of
our modifications: the Longfellow mDOC proof (Ligero) and the zkDilithium
signature proof (STARK). These are the references against which the cost of our
customizations is measured. As a sanity check, our desktop Longfellow numbers
($\sim$1.05\,s prove, $\sim$0.56\,s verify) are in the same range as the figures
...
(\cref{tab:runtimes_memory}).

\begin{table}[ht]
  \centering
  \caption{Unmodified building blocks. Median prove/verify time and proof size.}
  \label{tab:res-baselines}
  \small
  \begin{tabular}{@{}llrrr@{}}
    \toprule
    \textbf{System} & \textbf{Platform} & \textbf{Prove (ms)} & \textbf{Verify (ms)} & \textbf{Size (KB)} \\
    \midrule
    \multirow{3}{*}{Longfellow mDOC (Ligero)}
        & Desktop    & 1047 & 559  & 351.6 \\
        & Galaxy S24 & 973  & 530  & 351.6 \\
        & Galaxy A53 & 1822 & 1099 & 351.6 \\
    \midrule
    \multirow{3}{*}{zkDilithium (STARK)}
        & Desktop    & 225 & 33.5 & 173.7 \\
        & Galaxy S24 & 369 & 39.7 & 173.7 \\
        & Galaxy A53 & 432 & 40.6 & 173.7 \\
    \bottomrule
  \end{tabular}
\end{table}
...
big cores outperform a 2017-era Ryzen on this workload, which is encouraging for
a scheme meant to run on phones.

\subsection{Choosing the cross-system commitment: SHA-256 vs Poseidon}
\label{results:hash}
...
\subsection{Cost of the customizations}
\label{results:customizations}
We now isolate the cost added by each of our modifications, on top of the
baselines of \cref{tab:res-baselines}.

\Cref{tab:res-stark-steps} follows the STARK side. Adding the Poseidon23
commitment $R_S$ to the bare signature proof is almost free (and even within
noise on the desktop). The expensive step is the MAC witness-sharing bridge:
making the message $e_1$ a private witness, bit-decomposing it, recomputing the
GF$(2^{128})$ MAC $m_e$ in-circuit, and binding it to the signed message. This is
what inflates the STARK proof from $173.7$\,KB to $1.25$\,MB (a $7.2\times$
blow-up) and roughly doubles to triples the proving time. This blow-up is the
true price of post-quantum cross-system binding on the STARK side.

\begin{table}[ht]
  \centering
  \caption{STARK side: incremental cost of each customization. Median prove time
  / proof size.}
  \label{tab:res-stark-steps}
  \small
  \begin{tabular}{@{}lrrrr@{}}
    \toprule
    \textbf{STARK variant} & \multicolumn{3}{c}{\textbf{Prove (ms)}} & \textbf{Size} \\
    \cmidrule(lr){2-4}
     & Desktop & S24 & A53 & \textbf{(KB)} \\
    \midrule
    Baseline (message public)                 & 225 & 369 & 432  & 173.7 \\
    \;+ Poseidon23 commitment $R_S$           & 234 & 405 & 616  & 173.3 \\
    \;+ MAC witness sharing (final)           & 451 & 732 & 1382 & 1254 \\
    \bottomrule
  \end{tabular}
\end{table}

\Cref{tab:res-ligero-steps} follows the Ligero side. Adding the in-circuit
SHA-256 opening of $R_L=\SHA(k_p\,\|\,e_1)$ together with the $m_e$ MAC gadget
adds roughly $250$\,ms of proving time on the desktop while leaving the proof
size essentially unchanged ($\sim$335\,KB); the Ligero figures here are taken
from the split commit+finalize path used by the orchestrator. The Ligero side
therefore absorbs the cross-system binding far more gracefully than the STARK
side does.

\begin{table}[ht]
  \centering
  \caption{Ligero side: cost of adding the SHA-256 $R_L$ opening and the $m_e$
  MAC gadget. Median prove time / proof size.}
...
\begin{table}[ht]
  ...
  \begin{tabular}{@{}lrrr@{}}
    ...
    \midrule
    STARK proof size (KB)       & 1252 & 1252 & 1253 \\
    Ligero proof size (KB)      & 335  & 335  & 335  \\
    \textbf{Total proof (KB)}   & \textbf{1587} & \textbf{1587} & \textbf{1588} \\
    \bottomrule
  \end{tabular}
\end{table}
...
Three findings stand out. First, \textbf{post-quantum cross-system binding is
affordable}: the complete joint proof runs in about $1.1$\,s on a current
flagship and $2.2$\,s on a mid-range phone, with a total proof size of
${\sim}1.6$\,MB, while adding issuer post-quantum security and hiding the device
public key from the issuer. Second, \textbf{the cost is concentrated in the STARK
MAC bridge}, which alone accounts for the $7.2\times$ proof-size blow-up
(\cref{tab:res-stark-steps}); the Ligero side and the Poseidon commitment are
comparatively cheap. This identifies the STARK MAC gadget as the prime target for
the optimization work outlined in \cref{sec:future-work}. Third, the
...
Finally, the fact that the Galaxy~S24 matches or beats a desktop CPU, and that
even the mid-range A53 stays around two seconds, supports the central claim of
this thesis: privacy-preserving, post-quantum, device-bound anonymous credentials
are practical on the commodity hardware that the EUDI Wallet must actually run on.
...
\subsection{Formal Treatment and System Completion}
...
\subsection{Optimization of the Mixed Zero-Knowledge Construction}
...
\subsection{Extended Credential Functionality}
...
\subsection{Alternative and Exploratory Research Directions}
...
\subsection{Evolving the Threat Model}
...
\begin{itemize}
    \item \textbf{Advancement over the state of the art.} Existing deployable approaches, such as Frigo--Shelat, do not achieve post-quantum privacy and remain dependent on the secure element; pairing-based BBS/BBS+ offers elegant selective disclosure but is neither post-quantum nor hardware-compatible. A system that simultaneously targets privacy preservation, post-quantum security, device binding, and deployability on legacy hardware, and validates all four properties on commodity phones, would be the first of its kind.
    \item \textbf{Impact on European Digital Identity.} Every EU Member State is required to offer an EUDI Wallet, and the regulation's privacy goals are currently unmet. Directly addressing the two acknowledged weaknesses of the current design -- over-disclosure and linkability -- with real predicate proofs and unlinkable presentations, while making the wallet quantum-safe, would align it with both the EU post-quantum migration roadmap and the 2030 Digital Decade targets.
    \item \textbf{Societal benefits.} Citizens would gain genuine data minimization (for example, proving "older than 18" without revealing a birth date, or proving residence without revealing an address), protection from cross-service profiling, resilience against device loss through social recovery, and access regardless of phone tier, advancing both privacy-by-design and digital inclusion for hundreds of millions of users.
    \item \textbf{Economic and standardization impact.} Results would feed directly into EUDI ARF discussions, contributing to European sovereignty over a privacy-preserving, quantum-safe identity stack rather than importing one. An open-source library would lower the barrier for national wallet providers and for the private-sector relying parties the regulation aims to onboard, and would be reusable in adjacent settings such as Privacy Pass, remote attestation, and online age verification.
    \item \textbf{Deployment-friendliness.} Critically, this direction does not require citizens to purchase modern smartphones, nor does it require relying parties to adopt modern hardware. The only requirement on the ecosystem side is for the issuer to adopt a post-quantum, Dilithium-class standardized signature and to reduce the size of the device public-key field carried in the credential document, which, as a side effect, also yields a net space optimization in the document itself.
\end{itemize}x    
